...
(function($) {
	...
	window.Sitemap = function() {
		...
		var $topElement = $('body'),
			...
			_contents = '#sitemapWrap .sitemap',
			_anchor = '.bindSitemapSpread',
			_closer = '.bindSitemapFold',
			currentY = null,
			prevY = 0,
			scrollTop = 0,
			spread = false,
		...
		this.initialize = function() {
			...
			$topElement.on('click', _closer, this.fold);
		};
		...
		this.spread = function(e) {
			if(tween == false) {
				if(spread === false) {
					...
					TweenLite.fromTo($(_contents), 0.4, {y: '+=70%'}, {y: '0%', ease: Power0.easeNone});
					$(this).addClass('open');
				}
				tween = true;
			}
		...
		this.fold = function(e) {
			if(tween == false) {
				if(spread === true) {
					...
					window.removeEventListener('touchstart', _this.touchStart, false);
		...
		this.touchStart = function(e) {
			currentY = prevY = e.touches[0].clientY;
		};

		this.touchMove = function(e) {
			if($(e.target).is('#sitemapWrap *') === true) {
				currentY = e.touches[0].clientY;
				scrollTop = $(_contents).scrollTop();
				$(_contents).scrollTop(scrollTop + ((prevY - currentY)) * 2);
				prevY = currentY;
			}
			e.preventDefault();
		};
...
(function($) {
	$(document).ready(function() {
        ...
        TweenMax.set($('.nav .nav_a1'), {opacity: 0.2});
        TweenMax.to($('.nav .nav_a1'), 0.1, {opacity: 1, ease: "none", repeat: -1, yoyo: true, repeatDelay: 0.8});
